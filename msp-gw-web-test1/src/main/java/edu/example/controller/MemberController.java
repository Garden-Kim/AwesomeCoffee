...
public class MemberController {
	...
	public ModelAndView duplicate(HttpServletRequest request, HttpServletResponse response, HttpSession session) {
		...
		String dupYn = service.duplicate(reqBodyMap);
	...
	public ModelAndView find(HttpServletRequest request, HttpServletResponse response, HttpSession session) {
		Map<String, Object> reqHeadMap = (Map<String, Object>) request.getAttribute(Const.HEAD);
		Map<String, Object> reqBodyMap = (Map<String, Object>) request.getAttribute(Const.BODY);
		Map<String, Object> responseBodyMap = new HashMap<String, Object>();
		...
		String existYn = service.find(reqBodyMap);
	...
	public ModelAndView getMemberInfo(HttpServletRequest request, HttpServletResponse response, HttpSession session) {
		...
		if (StringUtils.isEmpty(loginDTO)) {
			...
			if (reqBodyMap.get("loginId").equals(loginDTO.getLoginId())) {
				MemberDto info = service.getMemberInfo(reqBodyMap);
				if (!StringUtils.isEmpty(info)) {
					responseBodyMap.put("rsltCode", "0000");
					responseBodyMap.put("rsltMsg", "Success");
					responseBodyMap.put("userNm", info.getUserNm());
					responseBodyMap.put("birthDate", info.getBirthDate());
					responseBodyMap.put("gender", info.getGender());
					responseBodyMap.put("cellPhone", info.getCellPhone());
